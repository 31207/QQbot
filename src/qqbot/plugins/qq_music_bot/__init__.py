"""校园广播站 QQ 点歌机器人（单一入口插件，仅私聊）。

消息处理顺序（单一 matcher，无隐式优先级耦合）：
1. 歌曲分享卡片 → 精确搜索 + 待确认点歌；
...
matcher = on_message(priority=1, block=True, rule=to_me() & Rule(_is_private))
...
async def send_result(bot: Bot, event: MessageEvent, result: ActionResult, phrase: str | None = None) -> None:
    if result.media is not None:
        await bot.send(event, MessageSegment.image(result.media))
    text = result.summary
...
async def dispatch(bot: Bot, event: MessageEvent) -> None:
    uid = event.get_user_id()
    text = event.get_plaintext().strip()
    user_state = state.user(uid)

    # 1) 歌曲分享卡片
    share = extract_share(event)
    if share is not None:
        await handle_share(bot, event, uid, user_state, share)
        return
    ...
    if cmd is not None and cmd.kind in ADMIN_KINDS:
        await run_admin_command(bot, event, uid, cmd)
        return
    t = (text or "").lstrip("/").strip()
    if t.startswith("封禁") or t.startswith("解封"):
        await bot.send(event, "用法：「封禁 用户ID」/「解封 用户ID」，仅超级管理员可用")
        return

    # 3) 封禁拦截
    if await users.is_banned(uid):
        await bot.send(event, "你已被封禁，无法使用点歌功能")
        return

    # 4) 状态流转
    if user_state.help_pending:
        user_state.help_pending = False
        if await handle_help_choice(bot, event, uid, user_state, text):
            return
    ...
    if user_state.pending_mode:
        ...
        if mode == "song" and text.strip().isdigit():
            await do_order(bot, event, uid, text, int(text.strip()))
            return
        if mode == "search" and text:
            await do_search(bot, event, uid, text, None)
            return
    ...
    if user_state.pending_order:
        ...
        if text.isdigit() or compact(text) in _CONFIRM_WORDS:
            ...
            await do_order(bot, event, uid, text, idx)
            return

    # 5) 确定性指令
    if cmd is not None:
        await run_command(bot, event, uid, user_state, cmd, text)
        return

    # 6) LLM 兜底
    if llm.enabled and text:
        await llm_fallback(bot, event, uid, text)


async def handle_share(bot: Bot, event: MessageEvent, uid: str, user_state, share: dict) -> None:
    ...
    if result.media is not None:
        ...
        await bot.send(event, result.summary or "没搜到这首歌，换个关键词或直接「搜索 歌名」试试。")


async def do_search(bot: Bot, event: MessageEvent, uid: str, raw_text: str, source: str | None) -> None:
    await bot.send(event, "正在搜索，请稍候…")
    result = await actions.action_search(uid, raw_text, source)
    await send_result(bot, event, result, phrase=raw_text)


async def do_order(bot: Bot, event: MessageEvent, uid: str, raw_text: str, index: int) -> None:
    result = await actions.action_order(uid, index)
    await send_result(bot, event, result, phrase=raw_text)


async def handle_help_choice(bot: Bot, event: MessageEvent, uid: str, user_state, text: str) -> bool:
    key = text.strip().lstrip("/")
    if key == "3":
        await send_result(bot, event, await actions.action_my_songs(uid))
        return True
    if key == "4":
        result = await actions.action_remaining(uid)
        await bot.send(event, result.summary)
        return True
...
async def run_command(bot: Bot, event: MessageEvent, uid: str, user_state, cmd: Command, raw_text: str) -> None:
    kind = cmd.kind

    if kind == CommandKind.SEARCH:
        source, keyword = cmd.args
        if not keyword:
            await bot.send(event, texts.SEARCH_USAGE)
            return
        await do_search(bot, event, uid, raw_text, source)
        return
    ...
    if kind in (CommandKind.SEARCH_NEXT, CommandKind.SEARCH_PREV, CommandKind.SEARCH_EXIT):
        ...
        await send_result(bot, event, await actions.action_paginate(uid, direction))
        return
    ...
    if kind == CommandKind.ORDER:
        ...
        if index is None:
            await bot.send(
                event, "用法：先「搜索 歌名」搜索歌曲，再「点歌 序号」点选结果，例如：点歌 3"
            )
            return
        await do_order(bot, event, uid, raw_text, index)
        return

    if kind == CommandKind.MY_SONGS:
        await send_result(bot, event, await actions.action_my_songs(uid), phrase=raw_text)
        return

    if kind == CommandKind.REMAINING:
        await send_result(bot, event, await actions.action_remaining(uid), phrase=raw_text)
        return
    ...
    if kind == CommandKind.REMARK:
        ...
        if song_id is None:
            ...
            return
        await send_result(
            bot, event, await actions.action_remark(uid, song_id, content or ""), phrase=raw_text
        )
        return

    if kind == CommandKind.HELP:
        user_state.help_pending = True
        await bot.send(event, texts.HELP_MENU)
        return


async def run_admin_command(bot: Bot, event: MessageEvent, uid: str, cmd: Command) -> None:
    kind = cmd.kind
    if kind == CommandKind.BAN_USER:
        ...
        await run_notice_send(bot, event, uid)
        return
    else:  # RESET_QUOTA
        result = await actions.action_reset_quota(uid)
    await send_result(bot, event, result)


async def run_notice_send(bot: Bot, event: MessageEvent, uid: str) -> None:
    if not permissions.is_admin(uid):
        await bot.send(event, "无权限：仅管理员可手动发送通知")
        return
    await bot.send(event, "正在发送待处理通知，请稍候…")
    stats = await _send_pending_notices()
    if stats is None:
        await bot.send(event, "当前没有已连接的 bot，无法发送，请稍后再试")
        return
...
async def llm_fallback(bot: Bot, event: MessageEvent, uid: str, text: str) -> None:
    state.push_memory(uid, "user", text)
    history = state.memory(uid)
    try:
        reply = await llm.run_tools(
            uid, history, emit=lambda media: bot.send(event, MessageSegment.image(media))
        )
    except Exception:
