...
- **查看自己的用户ID**：发 `ID`（或 `我的ID`）返回当前通道下自己的用户标识（封禁/白名单需按对应通道的 ID 配置）。
...
- **双适配器**：OneBot V11（NapCat）与官方 QQ（开放平台机器人）可同时接入；官方 QQ 同时支持 **C2C 单聊**与**频道私信（direct message）**，业务逻辑共用，收发按适配器自动适配（发图自动走文件上传 API）。用户标识随通道不同（QQ 号 / user_openid / 频道用户 id），封禁/白名单需按对应通道配置；歌曲分享卡片链路仅 NapCat 支持。
