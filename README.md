...
- **选用记录**：发 `选用记录`（或 `我的选用`/`已选歌曲`）查看自己点过的歌里**已被广播站选用**的歌曲图片，按选用时间倒序。
...
- **管理命令**：`封禁 用户ID` / `解封 用户ID` / `封禁列表`（仅超级管理员）；`禁歌 歌名/编号` / `解禁歌 歌名/编号` / `重置点歌次数`（仅管理员）；被封禁用户无法使用点歌功能。
...
- **双适配器**：OneBot V11（NapCat）与官方 QQ（开放平台机器人）可同时接入；官方 QQ 同时支持 **C2C 单聊**与**频道私信（direct message）**，业务逻辑共用，收发按适配器自动适配（发图自动走文件上传 API）。业务用户 ID 统一带平台前缀：`onebot:<QQ号>` / `c2c:<user_openid>` / `dms:<guild_id>:<频道用户id>`，封禁、白名单、点歌记录、选用记录均按该 ID 区分；歌曲分享卡片链路仅 NapCat 支持。
...
│   ├── services/                 # 业务服务（点歌/搜索/用户/权限/封面/选用记录/LLM）
...
```
- **用户**：可使用搜索、点歌、我的歌单、选用记录、剩余次数、备注、帮助。
